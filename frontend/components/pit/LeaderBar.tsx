 ...
 * is null while the 20s delay window hasn't cleared yet.
 */
export function LeaderBar({
  ...
  if (!bucket) {
    return (
      <div>
        <div className="flex h-3 w-full overflow-hidden border-2 border-brutal-fg">
          <div className="h-full w-1/2 bg-brutal-fg/10" />
        </div>
        <div className="mt-2 text-center text-xs uppercase tracking-wider text-brutal-fg/50">
          delayed — no read yet
        </div>
      </div>
    );
  ...
  return (
    <div>
      <div className="flex h-3 w-full overflow-hidden border-2 border-brutal-fg">
        <div
          className={`h-full transition-[width] duration-500 ${even ? "bg-brutal-fg/10" : "bg-emerald-500"}`}
          style={{ width: `${left}%` }}
        />
        <div
          className={`h-full transition-[width] duration-500 ${even ? "bg-brutal-fg/10" : "bg-red-500"}`}
          style={{ width: `${right}%` }}
        />
      </div>
      <div className="mt-2 text-center text-xs uppercase tracking-wider text-brutal-fg/50">
